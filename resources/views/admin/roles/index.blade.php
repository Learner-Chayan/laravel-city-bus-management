        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <a class="btn btn-primary uppercase text-bold float-right" href="{{ route('roles.create') }}"> New Role</a>
                    <div class="table-responsive" style="overflow-x: hidden">
                        <table id="example" class="table table-striped table-bordered zero-configuration">
                            <thead>
                                <tr>
                                    <th class="text-bold text-uppercase">#SL</th>
                                    <th class="text-bold text-uppercase">Role Name</th>
{{--                                    <th class="text-bold text-uppercase">Permission Name</th>--}}
...
{{--                                        <td>{{ str_replace(array('[',']','"'),'', $role->permissions->implode('name', ', ')) }}</td>--}}
                                        <td>
                                            <a class="btn btn-sm btn-primary fa fa-edit" href="{{ route('roles.edit',$role->id) }}"> Edit</a>

                                                @can('role-destroy')
                                                    {!! Form::button('<i class="fa fa-trash"></i> Delete', ['class' => 'btn btn-sm btn-danger bold uppercase delete_button','data-toggle'=>"modal",'data-target'=>"#DelModal",'data-id'=>$role->id]) !!}
                                                @endcan
